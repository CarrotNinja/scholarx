...
class AwardsPageScreen extends StatefulWidget {
  AwardsPageScreen({Key? key}) : super(key: key);

  @override
  State<AwardsPageScreen> createState() => _AwardsPageScreenState();
}

class _AwardsPageScreenState extends State<AwardsPageScreen>  {
  ...
  for (int i = 0; i < 8; i++) {
    String? academicValue = prefs.getString('awards$i'+user.email!);
        ...
        children: [
          ...
          CustomElevatedButton(
            ...
            onPressed: () async {
              SharedPreferences prefs = await SharedPreferences.getInstance();
              for(int i=0;i<8;i++){
                prefs.setString('awards$i'+user.email!, _fieldController[i].text);
              }
            }
          ),
        ],
